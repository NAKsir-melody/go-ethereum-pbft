...
// PbftDeveloperGenesisBlock returns the 'geth --pbftdev' genesis block. Note, this must
// be seeded with the
func PbftDeveloperGenesisBlock(period uint64, faucet common.Address) *Genesis {
	// Override the default period to the user requested one
	config := *params.AllPbftProtocolChanges
	config.Pbft.Period = period
