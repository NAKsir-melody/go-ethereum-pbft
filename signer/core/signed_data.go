...
var (
	...
	ApplicationPbft = SigFormat{
		accounts.MimetypePbft,
		0x03,
	}
	TextPlain = SigFormat{
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	...
	case ApplicationPbft.Mime:
		// Pbft 
		stringData, ok := data.(string)
		if !ok {
			return nil, useEthereumV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationPbft.Mime)
		}
		pbftData, err := hexutil.Decode(stringData)
		if err != nil {
			return nil, useEthereumV, err
		}
		header := &types.Header{}
		if err := rlp.DecodeBytes(pbftData, header); err != nil {
			return nil, useEthereumV, err
		}
		// The incoming Pbft header is already truncated, sent to us with a extradata already shortened
		if len(header.Extra) < 65 {
			// Need to add it back, to get a suitable length for hashing
			newExtra := make([]byte, len(header.Extra)+65)
			copy(newExtra, header.Extra)
		...
		// Get back the rlp data, encoded by us
		sighash, pbftRlp, err := pbftHeaderHashAndRlp(header)
		if err != nil {
			return nil, useEthereumV, err
		}
		message := []*NameValueType{
			{
				Name:  "pbft header",
				Typ:   "pbft",
				Value: fmt.Sprintf("pbft header %d [0x%x]", header.Number, header.Hash()),
			},
		}
		// Pbft uses V on the form 0 or 1
		useEthereumV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: pbftRlp, Message: message, Hash: sighash}
...
func pbftHeaderHashAndRlp(header *types.Header) (hash, rlp []byte, err error) {
	if len(header.Extra) < 65 {
		err = fmt.Errorf("pbft header extradata too short, %d < 65", len(header.Extra))
		return
	}
	rlp = pbft.PbftRLP(header)
	hash = pbft.SealHash(header).Bytes()
	return hash, rlp, err
}
